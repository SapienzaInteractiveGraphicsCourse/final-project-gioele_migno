...
class X_bot_Interpolation_Configuartions{
    ...
    __poses_to_animation_v2(start, end, time_start, time_end, animation){
        if(time_start == 0){
            animation.times.push(time_start);
            animation.values.push(start);
        }
        if(!this.utils.deepEqual(start, end)){
            animation.times.push(time_end);
            animation.values.push(end);
        }
    }
    ...
    _x_bot_configurations_to_animation_v2(array_config, array_time){
        if(array_time.length != array_time.length){
            console.log("ERROR: not same length");
            return;
        }
        ...
        for(let i=0; i<array_config.length-1; i++){
            let start = array_config[i];
            let end = array_config[i+1];

            let time_start = array_time[i];
            let time_end = array_time[i+1];

            for (const [key, value] of Object.entries(my_array_animation)) {
                this.__poses_to_animation_v2(start[key], end[key], time_start, time_end, my_array_animation[key].animation);
            }
        }
        return my_array_animation;
    }
